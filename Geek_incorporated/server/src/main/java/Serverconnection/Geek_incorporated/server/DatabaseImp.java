...
public class DatabaseImp implements Database
{
    ...
    {
        this.con = null;
        this.host = "jdbc:mysql://localhost:3306/Geek_incorporated";
        this.uName = "root";
        this.uPass = "password voor de mysql database op de server";
        this.rs = null;
    ...
    public long getBalance(String rekeningNr)
    {
        ...
        try
        {
            PreparedStatement ps = con.prepareStatement("SELECT saldo "
                                                           + "FROM Geek_incorporated.klant "
                                                           + "WHERE Geek_incorporated.klant.rekeningNummer = ?");
    ...
    public boolean withdraw(String rekeningNr, long amount)
    {
        ...
        try
        {
            if(saldo > amount)
            {
                ...
                PreparedStatement ps = con.prepareStatement("UPDATE Geek_incorporated.klant "
                                                               + "SET Geek_incorporated.klant.saldo = ? "
                                                               + "WHERE Geek_incorporated.klant.rekeningNummer = ?;");
